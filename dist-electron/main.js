...
import require$$1$3, { app, BrowserWindow, ipcMain, globalShortcut, Tray, Menu, screen, dialog } from "electron";
...
function saveFile(name, type2, file2, dir = DATADIR) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const base = type2 ? `${name}.${type2}` : name;
      const savePath = path$m.join(root2, base);
...
function getFile(name, dir = DATADIR) {
  return new Promise(async (resolve, reject) => {
    try {
      const root2 = resolveWorkspaceDir(dir);
      const filePath = path$m.join(root2, name);
...
async function renameWorkspaceFile(dir, oldRel, newName, newType) {
  try {
    ...
    const oldPath = path$m.join(root2, oldRel);
    const newPath = path$m.join(root2, newBase);
