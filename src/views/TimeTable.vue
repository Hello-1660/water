...
const TIMETABLE_FILE_TABLE = 'json'
...
const format2Table = (name: string, content: string): Table => {
...
const getTableList = async () => {
    ...
    const list: FileType[] = await window.electronAPI.openAllFiles(TIMETABLE_DIR)

    for (const item of list) {
        const content = await window.electronAPI.openFile(item.name + '.' + item.type, TIMETABLE_DIR)
        tableList.value.push(format2Table(item.name, content)) 
    }
}
...
<template>
    <div class="container">
        <div v-show="isShow === 0" class="show-table-list">
            <ul>
                <li class="add" @click="isShow = 1">
                ...
                <li v-if="tableList.length !== 0" v-for="(table, index) in tableList" :key="index">
                    {{ index }}
                </li>
            </ul>
        </div>
        ...
        <div v-show="isShow === 1" class="create-table">
            <div class="create-table-main">
                <div class="top">
                    <div class="return" @click="isShow = 0">
                ...
                <div class="table-option">
                    ...
                    <div class="table-item option">
                        ...
                        <ul>
                            <input type="text" v-model="itemContent" @keydown.enter="itemContentValue.push(itemContent); itemContent = ''">    
                        </ul>
                    </div>
                </div>
            </div>
